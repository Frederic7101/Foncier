...
vf_norm AS (
    SELECT DISTINCT
        code_dept,
        LEFT(LPAD(code_postal::text, 5, '0'), 5)                AS cp3,
        UPPER(REGEXP_REPLACE(unaccent(
            REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(
                REGEXP_REPLACE(
...
-- ── vf_communes : présence souple (dep + CP, sans nom) ───
...
COPY (
    ...
    LEFT JOIN foncier.geojson_communes gc ON gc.code_insee_geo = rn.code_insee
    ORDER BY rn.dep_code, rn.nom_standard
) TO 'C:/Users/frede/OneDrive/Documents/Cursor/diagnostic_completude_communes.csv'  -- adapter le chemin (psql CLI)
WITH (FORMAT CSV, HEADER true, DELIMITER ';', ENCODING 'UTF8');
