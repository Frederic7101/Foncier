...
BEGIN
  ...
  -- Nettoyage éventuel
  DROP TEMPORARY TABLE IF EXISTS tmp_vf_year;
  DROP TEMPORARY TABLE IF EXISTS tmp_vf_year_prev;

  /* =============================
     CREATION DE TABLES TEMPORAIRES (pour optimisation des perfs)
     ============================= */
  -- Données année N (p_year)
  CREATE TEMPORARY TABLE tmp_vf_year AS
  SELECT
    code_dept,
    code_postal,
    commune,
  ...
  ALTER TABLE tmp_vf_year
    ADD INDEX idx_tmp_year_group (code_dept, code_postal, commune, type_local),
    ADD INDEX idx_tmp_year_date  (date_mutation);

  -- Données année N-1 (p_year-1)
  CREATE TEMPORARY TABLE tmp_vf_year_prev AS
  SELECT
    code_dept,
    code_postal,
    commune,
  ...
  ALTER TABLE tmp_vf_year_prev
    ADD INDEX idx_tmp_prev_group (code_dept, code_postal, commune, type_local),
    ADD INDEX idx_tmp_prev_date  (date_mutation);
    ...
    (
      ...
      FROM (
        SELECT
          ...
          x.type_local,
          x.n,
          (x.n-1)*0.25+1 AS q1_pos,
          (x.n-1)*0.50+1 AS q2_pos,
          (x.n-1)*0.75+1 AS q3_pos,
      ...
      FROM (
        ...
        FROM (
          ...
          SELECT
            ...
            SUM(b4.prix) AS sum_prix,
            SUM(b4.surf) AS sum_surf,
            /* rangs pour médianes prix/surface */
            /* prix */
            ROW_NUMBER() OVER (
              PARTITION BY b4.code_dept, b4.code_postal, b4.commune, b4.type_local, b4.bucket
              ORDER BY b4.prix
            ) AS rn_prix,
            COUNT(*) OVER (
              PARTITION BY b4.code_dept, b4.code_postal, b4.commune, b4.type_local, b4.bucket
        ...
        JOIN (
          SELECT
            ...
            MAX(sum_prix) AS sum_prix,
            MAX(sum_surf) AS sum_surf
          FROM (
            SELECT
              b4.code_dept,
              b4.code_postal,
          ...
          ON t.code_dept = s.code_dept
         AND t.code_postal = s.code_postal
         AND t.commune = s.commune
         AND t.type_local = s.type_local
